###############################################################
# R Script - Calculate CSH and Clip to field plots
# 15-09-2017
# JW
###############################################################
...
tls_path <- "D:/Data_Processing/2017/W02/TLS/w2_15082017"

# Path for the DTM
dtm_path <- "D:/Data_Processing/2017/W02/TLS/w2_24052017"

# Path for CSH
csh_path <- "D:/Data_Processing/2017/W02/TLS_CSH/w2_15082017"

# Path for outputs
out_path <- "D:/Data_Processing/2017/W02/TLS_CSH/w2_15082017/out"

# Read DTM
setwd(dtm_path)
w2_dtm <- raster("w2_dtm_fill.tif")
cood <- projection(w2_dtm)
plot(w2_dtm)

# Read TLS point cloud
setwd(tls_path)
w2_tls <- read.table("w2_15082017_pc.pts",skip=1, 
                     row.names=NULL, col.names=c("x","y","z","i"))
head(w2_tls)

# Calculate CSH
tls_spdf <- w2_tls # the data frame converted to spatial point data frame (spdf)
coordinates(tls_spdf) <- ~x+y
proj4string(tls_spdf) <- cood

# Extract height value for respective Point Cloud x,y coordinates
dtm_z <- extract(w2_dtm,coordinates(tls_spdf))

# Calculate CSH (Actual Z - Terrain Z)
w2_tls$csh <- tls_spdf@data$z - dtm_z
w2_tls <- w2_tls[complete.cases(w2_tls), ] # Remove NA values
...
csh_df <- csh_df[, c("x","y","csh","i")] # Remove actual z column

# Write CSH to PTS file
setwd(csh_path)
write.table(csh_df, file="w2_15082017_pc_csh.pts", sep=" ", row.names=FALSE, col.names=FALSE)
...
# Writting rasters
setwd(out_path)
writeRaster(point_count, filename = "w2_15082017_csh_pointcount.tif", 
            format="GTiff", options="TFW=YES", overwrite=TRUE)
writeRaster(mean_csh, filename = "w2_15082017_csh_mean.tif", 
            format="GTiff", options="TFW=YES", overwrite=TRUE)
...
for(j in 1:length(fp)){
  ...
  temp_name <- paste("w2_15082017_", fpid[j], "_csh.csv", sep = "")
...
write.csv(nop_df, file = "w2_15082017_csh_npoints.csv", row.names = FALSE, quote = FALSE)
all_fp_csh <- all_fp_csh[,c("id","x","y","csh","i")]
all_fp_csh$id <- factor(all_fp_csh$id)
write.csv(all_fp_csh, file = "w2_15082017_all_csh.csv", row.names = FALSE, quote = FALSE)
